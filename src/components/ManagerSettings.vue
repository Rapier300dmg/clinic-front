<template>
  <div class="manager-settings">
    ...
    <div class="tabs">
      ...
      <button :class="{ active: activeTab === 'doctors' }" @click="activeTab = 'doctors'">
        Врачи
      </button>
    </div>
    <div class="tab-content">
      ...
      <div v-if="activeTab === 'doctors'">
        ...
        <button class="btn-add" @click="goToRegister">Добавить врача</button>
        <ul v-if="doctors.length">
          <li v-for="d in doctors" :key="d.id">
            Dr. {{ d.name || (d.firstName + ' ' + d.lastName) }} - {{ d.specialization }}
...
export default {
  ...
  methods: {
    ...
    async fetchDoctors() {
      try {
        const token = localStorage.getItem('token')
        // Если API для врачей отдельное, используйте его. Если врачи – часть users, отфильтруйте их.
