  ...
  methods: {
    ...
    async handleLogin() {
      ...
      try {
        ...
        console.log('Token payload:', payload) // Вывод payload в консоль для проверки

        // Используем альтернативные поля для идентификатора, если userId отсутствует
        const realId = payload.userId || payload.id || payload.sub
