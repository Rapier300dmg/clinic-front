    ...
    <div v-else>
      ...
      <div class="doctors-list">
        ...
        >
          ...
          <button class="btn-book" @click="goToBooking(doctor)">
            Записаться
          </button>
        </div>
        <p
          v-if="!filteredDoctors.length && !error"
          class="no-results"
        >
...
export default {
  ...
  methods: {
    goToBooking(doctor) {
      ...
      if (id) {
        // Переход на компонент подтверждения записи с передачей doctorId через параметры маршрута
        this.$router.push({
